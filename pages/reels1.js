...
export default function ReelsPage1() {
  ...
  // ===== LAZY LOADING STATES =====
  const [visibleReels, setVisibleReels] = useState([]); // Only reels to render
  const [loadedIndexes, setLoadedIndexes] = useState(new Set()); // Which indexes are loaded
  const [page, setPage] = useState(1); // Current page for pagination
  const [hasMore, setHasMore] = useState(true); // Whether more reels exist
  const [isLoadingMore, setIsLoadingMore] = useState(false); // Loading more reels
  const BUFFER_SIZE = 3; // Load current + next 2 reels (total 3)
  ...
  // Load only visible reels + buffer
  const loadVisibleReels = useCallback((currentIndex) => {
    if (!reels.length) return;
    
    const start = Math.max(0, currentIndex);
    const end = Math.min(reels.length - 1, currentIndex + BUFFER_SIZE - 1);
    
    // Create set of indexes to load
    ...
    // Set visible reels for rendering
    const visible = reels.slice(start, end + 1).map((reel, idx) => ({
      ...reel,
      originalIndex: start + idx // Store original position
    }));
    
    setVisibleReels(visible);
  }, [reels, BUFFER_SIZE]);
  ...
  const loadMoreReels = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === 'myreels') {
        // For "My Reels" tab, we need different logic
        const allReels = await reelService.getReels(nextPage, 20);
        ...
        data = await reelService.getReels(nextPage, 20);
      }
      ...
      if (newReels.length > 0) {
        ...
        // If this is the first load, set initial visible reels
        if (visibleReels.length === 0 && newReels.length > 0) {
          setTimeout(() => loadVisibleReels(0), 100);
        }
      } else {
        setHasMore(false);
      }
  ...
  }, [page, hasMore, isLoadingMore, loading, activeTab, user, visibleReels.length, loadVisibleReels]);

  // ===== USE EFFECTS =====

  // Initialize visible reels when reels are loaded
  useEffect(() => {
    if (reels.length > 0 && visibleReels.length === 0) {
      loadVisibleReels(0);
    }
  }, [reels, visibleReels.length, loadVisibleReels]);

  // Update visible reels when current reel changes
  useEffect(() => {
    loadVisibleReels(currentReelIndex);
  }, [currentReelIndex, reels, loadVisibleReels]);
  ...
  // Clean up video refs that are no longer visible
  useEffect(() => {
    // Clean up video refs for reels that are not loaded
    videoRefs.current.forEach((video, index) => {
      if (video && !loadedIndexes.has(index)) {
        video.pause();
        video.src = '';
        video.load();
  ...
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const originalIndex = parseInt(entry.target.getAttribute('data-original-index'));

            if (originalIndex !== currentReelIndex) {
              setCurrentReelIndex(originalIndex);

              // Mute and pause all other videos
              videoRefs.current.forEach((video, i) => {
                if (video && i !== originalIndex) {
                  video.muted = true;
                  video.pause();
                }
              });
              ...
              const currentVideo = videoRefs.current[originalIndex];
              if (currentVideo) {
    ...
    const reelContainers = document.querySelectorAll('[data-original-index]');
  ...
  }, [visibleReels.length, muted, audioBlocked, currentReelIndex]);
  ...
  const fetchReels = useCallback(async (filterType = 'foryou') => {
    try {
      ...
      setVisibleReels([]); // Clear visible reels
      setLoadedIndexes(new Set()); // Clear loaded indexes
      ...
      // Initialize visible reels after fetch
  ...
  }, [user, router, loadVisibleReels]);

  useEffect(() => { fetchReels(); }, [fetchReels]);

  // Handle scroll for current reel detection
  useEffect(() => {
    const handleScroll = () => {
      const reelContainers = document.querySelectorAll('[data-original-index]');
      ...
      reelContainers.forEach((container) => {
        const originalIndex = parseInt(container.getAttribute('data-original-index'));
  ...
  }, [currentReelIndex, visibleReels.length]);
  ...
  // Action handlers (updated to work with lazy loading)
  const handleLike = async (reel) => {
    ...
    try {
      ...
      // Update visible reel if it's currently visible
      setVisibleReels(prev => prev.map(v => 
        v._id === updatedReel._id ? { ...v, ...updatedReel } : v
      ));
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setVisibleReels(prev => prev.filter(v => v._id !== reelToDelete._id));
  ...
  const handleUploadReel = async () => {
    ...
    try {
      ...
      // Add to beginning of all states
      setReels(prev => [newReel, ...prev]);
      setVisibleReels(prev => [{
        ...newReel,
        originalIndex: 0
      }, ...prev.map(v => ({ ...v, originalIndex: v.originalIndex + 1 }))]);
  ...
  const getOptimizedVideoUrl = (originalUrl) => {
    ...
    return originalUrl.replace(
      '/upload/',
      dataSaver ? '/upload/q_auto:eco,f_auto:video/' : '/upload/q_auto:good,f_auto:video/'
    );
  };
  ...
  <div className="h-screen bg-black relative overflow-hidden">
    ...
    <div className="absolute top-0 left-0 right-0 z-50 bg-white bg-opacity-10 backdrop-blur-md p-3 flex items-center justify-between">
      ...
      <button
        onClick={() => {
          ...
          localStorage.setItem('dataSaverMode', newValue.toString());
          alert(`Data Saver ${newValue ? 'ON' : 'OFF'}. Reels will use less data.`);
        }}
        className="px-3 py-1.5 bg-purple-800 bg-opacity-60 text-white rounded-full text-xs font-semibold hover:bg-purple-900 transition flex items-center gap-1.5"
      >
        {dataSaver ? '🌿 ON' : '💧 OFF'}
    ...
    >
      {/* ✅ LAZY LOADING: Use visibleReels instead of reels */}
      {visibleReels.length === 0 && reels.length === 0 ? (
        ...
        // ✅ MAP visibleReels INSTEAD OF reels
        visibleReels.map((reel, localIndex) => {
          const originalIndex = reel.originalIndex;
          ...
          return (
            <div 
              ...
              // ✅ USE data-original-index FOR LAZY LOADING
              data-original-index={originalIndex}
            >
              {/* Video - Only render if loaded */}
              {isLoaded ? (
                <video
                  // ✅ USE originalIndex FOR REFS
                  ref={el => videoRefs.current[originalIndex] = el}
                  src={getOptimizedVideoUrl(reel.videoUrl)}
                  className="h-full w-full object-cover"
                  loop
                  muted={muted || audioBlocked || originalIndex !== currentReelIndex}
                  autoPlay={originalIndex === currentReelIndex}
                  onClick={() => {
                    togglePlay();
              ...
              <div className="absolute right-4 bottom-36 flex flex-col items-center space-y-6">
                ...
                {/* ✅ FIXED LIKE BUTTON - Check current likes state */}
                <button
                  onClick={() => handleLike(reel)}
                  disabled={actionLoading[reel._id]}
                  className="flex flex-col items-center"
                >
                  {/* Check both likedReels state AND reel.likes array */}
                  {likedReels[reel._id] || (user && reel.likes?.includes(user._id)) ? (
              ...
              {/* Play/Pause Overlay */}
              {!playing && originalIndex === currentReelIndex && (
                <button
                  onClick={togglePlay}
      ...
      {isLoadingMore && (
        <div className="h-32 flex items-center justify-center text-white">
          <div className="text-sm">Loading more reels...</div>
        </div>
      )}
      ...
      {!hasMore && reels.length > 0 && (
        <div className="h-32 flex items-center justify-center text-white">
          <div className="text-sm opacity-70">No more reels to load</div>
        </div>
      )}
    </div>

    {/* Progress Indicator - Use reels for total count */}
    {reels.length > 0 && (
