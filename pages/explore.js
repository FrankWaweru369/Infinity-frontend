...
export default function Explore() {
  const { isDarkMode } = useTheme(); // Add this to use dark mode
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-black pt-16">
      <div className="max-w-4xl mx-auto p-4">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-600 dark:from-infinityPurpleDark to-indigo-600 bg-clip-text text-transparent mb-2">
            Discover Infinity
          </h1>
          <p className="text-gray-600 dark:text-infinityTextSecondaryDark">Find amazing people and content</p>
        </div>

        {/* Enhanced Search Section */}
        <div className="bg-white dark:bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-lg p-6 mb-6 border border-gray-200 dark:border-infinityBorderDark">
...
/>
            ...
            <button
              ...
              disabled={isLoading}
              className="bg-gradient-to-r from-purple-600 dark:from-infinityPurpleDark to-indigo-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 disabled:opacity-50 flex items-center space-x-2 shadow-lg hover:shadow-xl"
            >
              {isLoading ? (
          ...
          {hasSearched && results.length > 0 && (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-500 dark:text-infinityTextSecondaryDark">Filter:</span>
        ...
        <div className="bg-white dark:bg-gray-900 dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          {isLoading ? (
            <div className="text-center py-12">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto mb-4"></div>
              <p className="text-gray-500 dark:text-infinityTextSecondaryDark">Searching {searchType}...</p>
            ...
            filteredResults.length > 0 ? (
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-gray-800 dark:text-infinityTextDark">
                ...
                {searchType === 'users' ? (
                  <div className="grid gap-4 md:grid-cols-2">
                    {filteredResults.map(user => (
                      <div key={user._id} className="bg-gradient-to-br from-white to-gray-50 dark:from-infinityCardDark dark:to-gray-900 border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-200 group">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            {getAvatar(user.profilePicture, user.username, 12, user.isOnline)}
                            <div>
                              <Link href={`/profile/${user.username}`} className="font-semibold text-gray-800 hover:text-purple-600 transition-colors group-hover:text-purple-600 dark:text-infinityPurpleDark">
                                {user.username}
                              </Link>
                          ...
                          <button 
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center space-x-2 ${
                              user.isFollowing
                                ...
                                : 'bg-gradient-to-r from-purple-600 dark:from-infinityPurpleDark to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 shadow-lg hover:shadow-xl'
                            }`}
                          >
                  ...
                  <div className="space-y-4">
                    {filteredResults.map(post => (
                      <div key={post._id} className="bg-gradient-to-br from-white to-gray-50 dark:from-infinityCardDark dark:to-gray-900 border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-200 group">
                        <div className="flex items-center space-x-3 mb-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500 dark:text-infinityTextSecondaryDark">
                              {new Date(post.createdAt).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                        <p className="text-gray-800 mb-4 leading-relaxed">{post.content}</p>
                        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-infinityTextSecondaryDark">
                ...
                discoveryData.users.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    <div className="w-24 h-24 mx-auto mb-4 bg-gradient-to-br from-purple-100 to-indigo-100 rounded-full flex items-center justify-center">
                      <FiUsers className="w-10 h-10 text-purple-600 dark:text-infinityPurpleDark" />
                    </div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">No users to discover yet</h3>
                    <p className="text-gray-500 dark:text-infinityTextSecondaryDark">Check back later for user recommendations</p>
                  </div>
                )
              ) : (
  ...
  <div className="grid gap-6">
    {discoveryData.posts.map(post => (
      <div key={post._id} className="bg-white dark:bg-gray-900 dark:bg-gray-900 border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition-all duration-300 group">
        ...
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div className="flex-1">
            <Link 
              ...
              className="font-semibold text-gray-900 hover:text-purple-600 transition-colors group-hover:text-purple-600 dark:text-infinityPurpleDark"
            >
              {post.author?.username}
            </Link>
        ...
        <div className="mb-4">
          ...
          {post.image && (
            <div className="mt-4 rounded-xl overflow-hidden border border-gray-200 dark:border-infinityBorderDark">
              <img 
                src={post.image} 
                alt="Post content" 
        ...
        <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
          <div className="flex items-center space-x-6">
            {/* Like Count */}
            <div className="flex items-center space-x-2">
              <FiHeart className="w-4 h-4 text-gray-400 dark:text-gray-500" />
              <span>{post.likesCount || post.likes?.length || 0}</span>
            </div>

            {/* Comment Count */}
            <div className="flex items-center space-x-2">
              <FiMessageCircle className="w-4 h-4 text-gray-400 dark:text-gray-500" />
  ...
  <div className="text-center py-12">
    <div className="w-24 h-24 mx-auto mb-4 bg-gradient-to-br from-purple-100 to-indigo-100 rounded-full flex items-center justify-center">
      <FiFileText className="w-10 h-10 text-purple-600 dark:text-infinityPurpleDark" />
    </div>
    <h3 className="text-lg font-semibold text-gray-800 mb-2">No posts to discover yet</h3>
    <p className="text-gray-500 dark:text-infinityTextSecondaryDark">Check back later for trending content</p>
  </div>
)              )}
            </div>
          )}
